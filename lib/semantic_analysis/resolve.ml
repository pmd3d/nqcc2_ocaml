...
let copy_identifier_map (m : var_entry StringMap.t) =
...
let rec resolve_type struct_map = function
  | Structure tag -> (
      try
        let unique_tag = (StringMap.find tag struct_map).unique_tag in
...
let resolve_local_var_helper id_map name storage_class =
  ...
  | Some { from_current_scope = true; has_linkage; _ } ->
      if not (has_linkage && storage_class = Some Extern) then
        (* variable is present in the map and was defined in the current
           block *)
...
and resolve_function_declaration struct_map id_map fn =
  ...
  | _ ->
      ...
      in
      let new_id_map = StringMap.add fn.name new_entry id_map in
...
and resolve_structure_declaration struct_map { tag; members } =
  let prev_entry = StringMap.find_opt tag struct_map in
  let new_map, resolved_tag =
    match prev_entry with
    | Some { unique_tag; struct_from_current_scope = true } ->
        (* this refers to the same struc we've already declared, don't update
           the map *)
    ...
    | _ ->
        ...
        (StringMap.add tag entry struct_map, unique_tag)
  in
  let resolve_member m =
    (* note that we need to use new structure map here in case member type is
       derived from this structure type *)
...
let resolve_file_scope_variable_declaration struct_map id_map
    ({ name; var_type; _ } as vd : variable_declaration) =
